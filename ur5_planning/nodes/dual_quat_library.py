...
class Quaternion(object):
    ...
    def get_axis_angle(self):
        new_q = self.total/np.sqrt(np.dot(self.total, self.total))
        # print(new_q)
        if new_q[0] == 1:
            # raise Exception("q0=1\n")
            ang = 0
            ax = np.array([1, 0, 0])
        else:
    ...
    def get_rotation_matrix(self):
        nq = self.total / np.sqrt(np.dot(self.total, self.total))
    ...
    # Return a new copy of the quaternion
    def copy_quat(self):
        return Quaternion(self.total)
...
class DualQuaternion(object):
    ...
    def retrieve_position_vec(self):
        temp = self.dual * self.real.conjugate_quat()
        return 2*temp.vector

    # Retrieve Rotation Matrix
    def retrieve_rotation_matrix(self):
        return self.real.get_rotation_matrix()

    # Return a new copy
    def copy_new(self):
        return DualQuaternion(self.real.copy_quat(), self.dual.copy_quat())
...
class IntpRigidDispDualQuat(object):
    ...
    def __init__(self, *argv):
        if len(argv) == 4:
            self.initial_config = self.convert_dual_quat(argv[0], argv[1])
            self.final_config = self.convert_dual_quat(argv[2], argv[3])
        elif len(argv) == 2:
            self.initial_config = argv[0]
            self.final_config = argv[1]
        else:
            raise Exception("Number of possible inputs does not match requirement")
        self.relative_config = self.initial_config.get_conjugate() * self.final_config
        temp_dir = self.relative_config.dual.vector
        self.translation_dir = temp_dir/np.sqrt(np.dot(temp_dir, temp_dir))

    # Given a Dual-Quaternion return its Screw parameters(u, th, d, m)
    def compute_screw_param(self, q):
        u, th = q.real.get_axis_angle()
        if th == 0:
            u = self.translation_dir
    ...
    def interpolate_one_step(self, tau=0.1):
        sp = self.compute_screw_param(self.relative_config)
        return self.initial_config * self.pow_dual_quat(sp, tau)
...
# Homogeneous matrix to Dual-Quaternion representation
def homo2dualquat(hmat):
    q = rotm2quat(hmat[:3, :3])
    p = hmat[:3, 3].flatten()
    real = Quaternion(q)
    u, th = real.get_axis_angle()
    scalar_part = -np.sin(th / 2) * np.dot(p, u)
